...
export function BatchWelcome() {
  ...
  const handleCreate = async () => {
    if (!name.trim()) return;
    const proj = await createProject(name.trim(), undefined, "batch");
  ...
  return (
    <div style={styles.root}>
      <div style={styles.card}>
        <div style={{ ...styles.logo, ...styles.logoAnimated }}>B</div>
        <h1 style={{ ...styles.title, ...styles.titleAnimated }}>
          Cyrano Batch
        </h1>
        <p style={{ ...styles.subtitle, ...styles.subtitleAnimated }}>
          Mine viral clips from your footage
        </p>
